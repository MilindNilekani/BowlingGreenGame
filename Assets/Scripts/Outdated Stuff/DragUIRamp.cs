...
public class DragUIRamp : MonoBehaviour,IBeginDragHandler,IDragHandler,IEndDragHandler {

	private GridHandler _grid;
	public GridHandler.FrictionValue _frictionVal;
	...
	public void OnEndDrag(PointerEventData data)
	{
		...
		if (ConstantHandler.Instance.ComponentAdded) {
			...
			if (_grid.isOnGrid (pos.x - 1, pos.y) && _grid.GetTileType(pos.x-1,pos.y)==GridHandler.Item.EMPTY) {
				_grid.SetTileToType (pos.x - 1, pos.y, GridHandler.Item.RAMP_NOTHING);
				_grid.SetDirectionOfTile (pos.x, pos.y, GridHandler.ItemDirection.LEFT);
				_grid.AttachItem (pos.x - 1, pos.y);
			} else if (_grid.isOnGrid (pos.x, pos.y + 1) && _grid.GetTileType(pos.x,pos.y+1)==GridHandler.Item.EMPTY) {
				_grid.SetTileToType (pos.x, pos.y + 1, GridHandler.Item.RAMP_NOTHING);
				_grid.SetDirectionOfTile (pos.x, pos.y, GridHandler.ItemDirection.UP);
				_grid.AttachItem (pos.x, pos.y + 1);
			} else if (_grid.isOnGrid (pos.x + 1, pos.y) && _grid.GetTileType(pos.x+1,pos.y)==GridHandler.Item.EMPTY) {
				_grid.SetTileToType (pos.x + 1, pos.y, GridHandler.Item.RAMP_NOTHING);
			...
			//_grid.SetTileToType (pos.x, pos.y, GridHandler.Item.RAMP);
			/*_grid.SetFrictionForTile (pos.x, pos.y, _frictionVal);
